...
| `onChange`              | Non         | <code>(key?: string, value?: TSource) =&gt; void</code>                                                                | Appelé avec la clé correspondante lors de la sélection d'une valeur.<br /><br />Sera appelé avec `undefined` (si `allowUnmatched = false`) si aucune suggestion n'est disponible lors de la confirmation de la saisie<br />(au blur du champ ou en appuyant sur Entrée). |
