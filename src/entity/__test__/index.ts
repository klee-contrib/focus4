...
function getStore() {
    const subStore = makeEntityStore(
        ...
        [OperationEntity, StructureEntity],
        {
    ...
    return makeEntityStore(
        ...
        [OperationEntity, ProjetEntity, StructureEntity, LigneEntity],
        {
            projetTest: "projet",
...
test("EntityStore: Ajout élément dans une liste", t => {
    ...
    t.deepEqual(
        store.structureList[3].id.$entity,
...
test("ViewModel: Création", t => {
    ...
    t.deepEqual(
        viewModel.numero,
        entry.numero,
        "Les champs simples du viewModel sont bien identiques à ceux du model."
    );
    t.deepEqual(
        viewModel.structure,
        entry.structure,
        "Les champs composites du viewModel sont bien identiques à ceux du model."
    );
    t.assert(isObservableArray(viewModel2.ligneList), "Une sous liste est bien toujours observable");
    t.deepEqual(
        viewModel2.ligneList.$entity,
        entry2.ligneList.$entity,
        "Une sous liste a bien toujours son entité attachée."
    );
    t.assert(
        !isObservable(viewModel2.ligneList.$entity),
        "Le champ '$entity' d'une sous liste n'est bien pas observable"
    );
    ...
    t.equal(
        viewModel.id.value,
        entry.id.value,
        "Les modifications du model sont bien répercutées sur les champs simples."
    );
    t.deepEqual(
        viewModel.structure,
        entry.structure,
        "Les modifications du model sont bien répercutées sur les champs composites."
    );
    ...
    t.equal(
        ...
        operation.structure.id,
        "Champ composite via set global: le model est bien toujours identique."
    );
    ...
    t.equal(
        ...
        "Champ composite via set local: le model est bien toujours identique."
    );
    ...
    t.equal(
        viewModel.structure.id.value,
        26,
        "Champ composite via set global: le viewModel est bien toujours identique."
    );
    t.equal(entry.structure.id.value, 26, "Champ composite via set global: le model a bien été mis à jour.");
    t.equal(
        viewModel.structure.nom.value,
        "yolo",
        "Champ composite via set local: le viewModel est bien toujours identique."
    );
    ...
    t.equal(
        viewModel.structure.nom.value,
        "yolo",
        "Champ composite via set local: le viewModel a bien été réinitialisé."
    );
