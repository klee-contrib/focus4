...
### `disableInlineSizing` / `labelRatio` / `valueRatio`
...
-   `labelRatio` permet de définir le ratio du composant de libellé (par défaut à 33%).
-   `valueRatio` permet de définir le ratio du composant de valeur (par défaut à 67%).
-   `disableInlineSizing` permet de désactiver complètement ces ratios (qui sont posés en CSS inline), pour les remplacer par un CSS personnalisé.
...
Permet de surcharger le handler appelé lors de la modification du champ par le composant de saisie. Si non renseigné, la valeur du champ sera directement
mise à jour avec la nouvelle valeur issue du composant. Vous pouvez utiliser cette option afin de personnaliser ce comportement.

_Remarque : Si vous souhaitez conserver le comportement par défaut de `onChange` qui met à jour le champ, **vous devez réimplémenter l'affectation dans
votre `onChange` personnalisé**. N'oubliez donc pas de faire `field.value = value` à l'intérieur._

### `autocompleteProps` / `displayProps` / `labelProps` / `inputProps` / `selectProps`

Vous pouvez surcharger les props des composants de domaine ici. Elles s'ajouteront aux props déjà définies dans le domaine et aux surcharges existantes dans
