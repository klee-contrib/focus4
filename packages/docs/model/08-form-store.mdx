...
_Remarque : Si vous avez (encore) des composants classe, vous pouvez utiliser `makeFormNode`, qui prend les mêmes paramètres que `useFormNode` mais
avec `this` en plus en premier. Vous pouvez aussi créer le noeud directement avec `Form(List)NodeBuilder`, en n'oubliant pas d'appeler `build()` et de
gérer l'appel à `dispose()` sur le `FormNode` pour désactiver la réaction de synchronisation._
