...
Les propriétés calculées **`errors`** et **`isValid`** sont ajoutées sur chacun des `field` et contiennent le résultat de la validation du champ. En particulier, `fieldFor`/`selectFor`/`autocompleteFor` récupère directement le résultat de `field.errors` pour afficher les erreurs dans les composants du champ.
...
De plus, si un noeud a été défini comme `isRequired: false`, alors tous ses champs et sous-noeuds seront également considérés comme `isRequired: false` si **le noeud est entièrement vide** (tous les champs à `undefined`, et les listes vides). `getValues` retournera `undefined` pour
un noeud vide non obligatoire, au lieu d'un objet vide (`{}`).

_Remarque : un champ non éditable (`field.isEdit == false`) est toujours valide (`field.isValid == true`), quelque soit le résultat de la validation._
...
En plus des quatre propriétés `hasChanged`, `isEdit`, `errors` et `isValid` vues précédement, `FormNode` et `FormListNode` exposent aussi :
