...
-   Passer `forceErrorDisplay` aux champs posés à l'intérieur pour forcer l'affichage des erreurs à la première sauvegarde.
-   Configurer `labelRatio`, `valueRatio` et `disableInlineSizing` pour tous les champs posés à l'intérieur.

## `Panel`

Il s'agit du composant [`Panel`](/docs/composants-focus4∕forms-panel--docs) standard, pour lequel, comme pour `Form`, l'objet d'actions expose une propriété dédiée **`panelProps`**, avec les propriétés suivantes du Panel :
