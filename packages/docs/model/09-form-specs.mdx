import {Meta} from "@storybook/addon-docs/blocks";

<Meta title="Modèle métier/Spécifications de formulaire" />

# Spécifications de formulaire

Puisque les définitions de domaines, champs et entités sont définies globalement dans l'application (et pour les deux dernières a priori générées), on a très souvent besoin de pouvoir **modifier ces définitions** lors de la création du formulaire. On pourrait par exemple avoir besoin de :

- Définir un état d'édition initial
- Désactiver l'édition sur un ou plusieurs champs
- Rendre certains champs obligatoires (ou non)
- Implémenter des validateurs supplémentaires
- Modifier des métadonnées comme un libellé ou un composant d'affichage/saisie
- Rendre n'importe lequel de ces exemples dépendant de la valeur d'un champ en particulier
- ...

Cela se fait via **le deuxième paramètre de `useFormNode`** qui est un "builder" qui va permettre de reconfigurer toutes ces définitions.

## `FormNodeBuilder`

Il s'agit du builder qui va s'appliquer sur un `StoreNode`, via `useFormNode()`, ou `patch()` et `items()` qui sont décrits plus bas.

Il dispose des méthodes suivantes :

### `edit(value)`

Permet de contrôler l'état d'édition du noeud. Elle fonctionne comme [`edit()`](/docs/modèle-métier-makefield--docs#editvalue) sur un champ.

`value` peut donc valoir :

- `true` ou `false`, selon l'état souhaité.
- Une fonction retournant `true` ou `false`. Utiliser une fonction permettra de **forcer l'état d'édition du noeud** à la valeur retournée par la fonction, au lieu de la rendre librement modifiable.

### `edit(value, ...props)`

`edit()` dispose d'une surcharge permettant de passer des propriétés du `StoreNode` en paramètres. Dans ce cas, **la valeur initiale/fonction sera appliquée aux champs/listes/objets demandés au lieu de l'objet en lui-même**. Il est donc parfaitement possible d'utiliser `edit` plusieurs fois, tant que ça s'applique à des propriétés différentes.

Exemple :

```ts
s.edit(!router.state.proId) // objet en entier
    .edit(false, "id", "dateCreation", "dateModification") // valeur par défaut sur certains champs
    .edit(() => !router.state.proId, "typeProfilCode"); // édition forcée sur un champ
```

La façon standard de modifier l'état d'édition d'un membre d'objet est de passer par `patch` (ou `add`), décrits plus bas, mais cette version de `edit()` permet de spécifier le même état d'édition à plusieurs champs à la fois. C'est aussi une version plus courte pour une seule propriété si l'édition est la seule chose à modifier dessus.

### `required(value)`

Permet de surcharger le caractère obligatoire d'un noeud. Comme pour `edit()`, `value` peut valoir :

- `true` ou `false`, selon l'état souhaité.
- Une fonction retournant `true` ou `false`, pour que le caractère obligatoire soit calculé.

### `required(value, ...props)`

Comme `edit()`, `required()` dispose également d'une surcharge permettant de passer des propriétés du `StoreNode` en paramètres. Dans ce cas, **la valeur ou la fonction sera appliquée aux champs/listes/objets demandés au lieu de l'objet en lui-même**. Il est donc parfaitement possible d'utiliser `required` plusieurs fois, tant que ça s'applique à des propriétés différentes.

Exemple :

```ts
s.required(false) // noeud en entier
    .required(() => !router.state.proId, "nom", "prenom"); // surcharge identique sur deux champs du noeud.
```

Pour des sous-noeuds, `required()` appellera le `required()` du builder du sous-noeud, tandis que pour un champ, il posera un [`metadata({isRequired})`](/docs/modèle-métier-makefield--docs#metadatametadata) ou un [`metadata(() => ({isRequired}))`](/docs/modèle-métier-makefield--docs#metadatametadata), selon le type de `value`.

### `add(name, entryBuilder)`

La fonction `add` permet d'ajouter un nouveau membre au `FormNode`, qui n'existe pas dans le noeud source. Cela peut donc être :

- Un nouveau champ (en renseignant un domain avec `.domain()`, ou une définition de champ avec `.field()`, en premier dans le builder, qui deviendra un [`EntityFieldBuilder`](/docs/modèle-métier-makefield--docs#entityfieldbuilder) comme [`makeField`](/docs/modèle-métier-makefield--docs#makefieldname-builder))
- Un nouveau sous-noeud (en renseignant une entité dans `.object()` en premier dans le builder, qui deviendra un `FormNodeBuilder`)
- Un nouveau sous-noeud liste (en renseignant une entité liste dans `.list()` en premier dans le builder, qui deviendra un `FormListNodeBuilder`)

```ts
const formNode = useFormNode(
    {},
    e =>
        e
            .add("field", f => f.domain(DO_NUMBER)) // Ajoute un champ "field", qui pourra ensuite être personnalisé comme n'importe quel champ existant.
            .add("subObject", f => f.object(SubObjectEntity)) // Ajoute un sous-noeud "subObject"
            .add("subList", f => f.list(SubObjectEntity)) // Ajoute un sous-noeud list "subList"
);
```

Un membre ajouté dans un noeud de formulaire se comportera comme un membre classique au sein du noeud de formulaire, et sera mis à jour par les actions de ce noeud (`set`, `replace` et `clear`). En revanche, n'étant pas lié à un membre correspondant dans le noeud source, **il ne sera pas affecté par les modifications du noeud source**. En particulier, un appel de `replace` ou `clear` sur le noeud source ne videra **pas** ce membre.

De plus, un membre ajouté sera **omis par défaut du résultat de [`getValues`](/docs/modèle-métier-stores-d-entité--docs#apis-de-storenode-et-storelistnode)**. L'usage principal étant porté par le service de sauvegarde des formulaires, il est plus cohérent d'omettre ces champs qui n'ont par définition pas de persistance dans le modèle. Il est possible cependant de les inclure en renseignant le second paramètre (optionnel) de `getValues` à `true`.

Il ne sera pas non plus pris en compte dans le calcul de `isEmpty` sur le noeud parent, et la valeur de son `hasChanged` sera toujours `false`.

### `patch(name, builder)`

La fonction `patch` permet de modifier un membre du `FormNode`, que ça soit un champ, une sous-liste ou un sous-objet. Elle prend comme paramètres :

- `name`, qui est le nom du champ/sous-objet/sous-liste. **Ce nom est typé et changera la signature en fonction de ce à quoi il correspond**.
- `builder`, qui peut être un `fieldBuilder`, `listBuilder` ou un `objectBuilder` en fonction du membre choisi. Ces fonctions seront appelées avec le builder correspondant (`EntityFieldBuilder`, `FormNodeBuilder` ou `FormListNodeBuilder`), ainsi que le `FormNode` courant.

### `remove(...fields)`

La fonction `remove` permet de supprimer les membres passés en paramètres du `FormNode`. Les membres supprimés existeront toujours sur le noeud source.

La suppression de membres permet de créer un formulaire qui ne concerne qu'une partie des champs d'une entité "complexe", **qui ne seront pas envoyés au service de sauvegarde**. Vous pouvez toujours retrouver les membres originaux supprimés du formulaire dans le `sourceNode`.

Par exemple :

```ts
const entity = useFormNode(profil, a => a.remove("dateCreation", "dateModification"));

return (
    <>
        {fieldFor(entity.libelle)}
        {fieldFor(entity.sourceNode.dateCreation)}
        {fieldFor(entity.sourceNode.dateModification)}
    </>
);
```

De cette façon, vous n'aurez même pas besoin de définir les champs supprimés comme non éditables puisqu'ils ne sont même pas dans le formulaire.

### `removeAllBut(...fields)`

La fonction `removeAllBut` fonctionne comme `remove`, sauf qu'elle permet à la place de supprimer tous les membres du `FormNode`, à l'exception de ceux passés en paramètres.

### `patchAllTo(entity)`

La fonction `patchAllTo` permet de modifier tous les membres d'un noeud pour qu'ils correspondent à ceux d'une autre entité. Les membres manquants sur le noeud source seront ajoutés, tandis que les membres en trop seront retirés. Si un membre a le même nom dans les deux entités, il sera patché (au lieu d'être supprimé puis recréé) si :

- Pour un champ, si le champ n'a pas déjà été patché avant et que le membre sur la nouvelle entité est bien un champ. Le patch remplacera entièrement la définition de l'ancien champ par celle du nouveau.
- Pour un sous-noeud, si le membre sur la nouvelle entité est bien un sous-noeud de la même entité. Le patch mettra simplement à jour le `required`.
- Pour un sous-noeud liste, si le membre sur la nouvelle entité est bien un sous-noeud liste de la même entité. Le patch mettra simplement à jour le `required`.

Dans l'idée, vous pouvez utiliser cette fonction pour transformer votre entité de lecture (qui contient les données chargées du serveur) en votre entité d'écriture, qui est celle qui contient les données effectivement à saisir par l'utilisateur, et qui donc doit être elle de votre formulaire.

Par exemple :

```ts
// Le type du formNode ici sera FormNode<ProfilWriteEntity, ProfilReadEntity>
const formNode = useFormNode(ProfilReadEntity, e => e.patchAllTo(ProfilWriteEntity));
// Le formNode n'aura ici non plus les champs `dateCreation` ou `dateModification` de l'entité de lecture (comme avec `remove`, mais de manière implicite).
```

## `FormListNodeBuilder`

Il s'agit du builder qui va s'appliquer sur un `StoreListNode`, via `useFormNode()` ou `patch()`.

Il dispose des méthodes suivantes :

### `edit(value)`

Idem `FormNodeBuilder`

### `required(value)`

Idem `FormNodeBuilder`

### `items(objectBuilder)`

La fonction `items` permet de modifier les items de la liste (qui sont, pour rappel, des `FormNode`). Elle prend comme unique paramètre `objectBuilder`, pour préciser la configuration. Comme toujours, cette fonction est donc appelée avec un `FormNodeBuilder` ainsi que le `FormListNode` courant.

## Exemple de spécifications fixes

```ts
const entity = useFormNode(utilisateurStore.utilisateur, e =>
    e
        .remove("id", "dateCreation", "dateModification")
        // Les composants de saisie qui fonctionnent avec plusieurs types (par exemple 'ZodString' ou 'ZodNumber')
        // doivent être renseignés avec le type générique correspondant.
        .patch("typeUtilisateurCode", f => f.metadata({SelectComponent: SelectRadio<ZodString>}))
        .add(
            // Ce champ ajouté ne sera pas transmis au serveur lors de la sauvegarde
            "nombreUtilisateursMax",
            f =>
                f
                    .domain(DO_ENTIER)
                    .metadata({
                        label: "Nombre maximal d'utilisateurs",
                        InputComponent: Slider,
                        inputProps: {labeled: true, ticks: true, max: 20},
                        // Le schéma du domaine est modifié pour y ajouter une valeur max.
                        schema: s => s.max(20)
                    })
                    .value(8) // Valeur initiale.
        )
);
```

## Spécifications dynamiques

L'ensemble de ces personnalisations peut aussi être **dynamique**, en référençant d'autres champs du formulaire ou d'autres états observables externes (et **observables uniquement**). Il faudra donc bien utiliser les versions des méthodes qui prennent des **dérivations** en paramètre (pour `edit()`, `metadata()` et `value()` en particulier).

_Remarque : La limitation sur les dépendances à des états observables dans la personnalisation vous forcera donc à utiliser `useLocalObservable()` plutôt que `useState()` dans vos composants, si vous avez besoin d'y référencer un état local._

Le noeud courant pourra être récupéré dans un paramètre supplémentaire à l'appel de `edit()`, `add()`, `patch()` et `items()`, afin de pouvoir définir simplement des personnalisations qui dépendent de l'état d'un champ du formulaire.

Par exemple :

```ts
useFormNode(utilisateurStore.utilisateur, f =>
    f
        // Puisque passer une dérivation à edit() force l'état d'édition, on peut créer un formulaire en édition permanente.
        .edit(() => true)
        .edit(() => !!router.state.id, "nom", "prenom")
        .patch(
            "email",
            // Pour accéder au formulaire, il faut récupérer `node` ici
            (f, node) =>
                // metadata prend bien ici une dérivation qui dépend de `node.prenom.value`
                f.metadata(() => ({isRequired: !!node.prenom.value}))
        )
        .add("length", (f, node) =>
            f // Champ calculé avec un validateur dessus qui s'ajoutera à ceux des champs.
                .domain(DO_ENTIER)
                .metadata({validator: {type: "number", max: 50}})
                .value(() => node.email.value?.length ?? 0)
        )
);
```

## Valeurs initiales

À la création du `FormNode` ou `FormListNode`, **il est vide**. En particulier, il ne recopie **pas** le contenu du store à partir duquel il a été créé par défaut. Il est cependant possible de renseigner des valeurs initiales :

- En renseignant les divers `value()` des champs qui le composent avec des `add()` ou des `patch()`
- En renseignant **le troisième paramètre de `useFormNode`** avec :
    - Un objet contenant les valeurs initiales du formulaire.
    - Une fonction retournant les valeurs initiales du formulaire (comme pour `useState` dans React).
    - `true` pour récupérer le contenu du store original.

    _Remarque : Ces valeurs s'appliqueront après celles définies dans les `value()`._

L'appel à `reset()` sur le `FormNode` ou `FormListNode` restaurera cet état initial, quelles que soient les modifications effectuées sur le store.

En général, si vous avez besoin d'initialiser le store avec les données issues du serveur, le chargement se fera dans le même composant qui définit le formulaire (et donc **après** qu'il soit construit). Par conséquent, le chargement qui mettra à jour le `StoreNode` ou `StoreListNode` mettra aussi à jour le `FormNode` ou `FormListNode` associé via la synchronisation assurée par `useFormNode`.
