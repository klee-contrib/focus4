...
- `load()` : Appelle le service de chargement du formulaire.
...
Pour définir la sauvegarde d'un formulaire, il existe deux possibilités :
...
Les deux possibilités sont mutuellement exclusives, vous ne pouvez pas définir `save` en même temps que `create` ou `update`.
...
- Si le service a retourné des données, le noeud source est mis à jour avec ce que le service a renvoyé, sinon on le met à jour avec les données du formulaire.
...
La signature du service d'`update` doit inclure les `params` en premier paramètre, puis le type du formulaire.
...
La fonction `on` est étendue pour les actions de formulaires et propose de définir des handlers pour les événements suivants en plus de `"load"` :
