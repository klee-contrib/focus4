...
Le type Typescript de Focus qui représente un champ est le type `FieldEntry`. C'est un type générique avec 8 paramètres (!!) parce qu'il doit contenir (dans
l'ordre) :
...
Vous n'avez en général pas à vous soucier de ce "détail" d'implémentation et de la complexité apparente des types de champs. Vous risquez cependant de le
voir passer au détour de certains messages d'erreurs Typescript lorsque vous ne passez pas les bons champs aux bons endroits...
...
Il n'est pas vraiment conseillé d'écrire ces définitions à la main, car cela serait fastidieux et sujet à erreurs, en particulier car pour la majeure partie d'entre-elles
elles doivent correspondre à une API de votre serveur.

Vous devriez utiliser un outil de modélisation externe pour les générer. Il parait que [TopModel](https://klee-contrib.github.io/topmodel) est un outil idéal pour ça et il dispose d'un [générateur](https://klee-contrib.github.io/topmodel/#/generator/js) dédié pour
ces entités Focus 😉.
...
_Remarque 2 : pour que le générateur TopModel génère `isRequired` sur les compositions (ce qui n'est pas un `"field"`), il faut activer l'option
`extendedCompositions`. Elle ajoutera aussi `label`, mais cette propriété n'est pas utilisée par Focus. Focus accepte que `isRequired` ne soit pas
renseigné, dans ce cas il sera à `true` par défaut._
