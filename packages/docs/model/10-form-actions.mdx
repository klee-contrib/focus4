...
Les actions du formulaire se définissent avec **`useFormActions`**, qui est un **superset de [`useLoad`](/docs/modèle-métier-chargement-des-données--docs#useload-et-nodeload)** qui s'utilise sur un [`FormNode`](/docs/modèle-métier-stores-de-formulaire--docs) pour y ajouter des services
pour la sauvegarde.
...
façon que pour `LoadRegistration`.
...
-   Vous pouvez définir un **service de création** (`builder.create()`) et/ou un **service de mise à jour** (`builder.update()`). `create` sera appelé à la
    sauvegarde **si les `params` ne sont pas définis**, et `update` s'ils le sont. `update` sera appelé avec les params, puis le contenu du noeud de formulaire.
...
Renseigner `init` permettra d'activer la méthode `actions.init()`, qui sera appelée à la place de `load` au premier rendu s'il n'y pas de `load` à appeler
(non défini, ou avec `params` à `undefined`).

La méthode effectuera un `set` des données retournées par le service en paramètre sur le `formNode` si défini (et non un `replace` sur le `storeNode`
comme `load`), puis écrasera les données du noeud source par les données du formulaire (avec un `replace` cette fois-ci). Ainsi, cela n'écrasera pas les
données initiales du formulaire qui ont pu préalablement être définies dans `useFormNode`, mais les complètera avec des données qui auront besoin d'être
...
Idéalement, tout formulaire utilisé sans `load` devrait appeler `init`. Il est cependant nécessaire de l'appeler explicitement, puisque le `load` n'est pas
nécessairement défini dans le `useFormActions` en cours.
...
Permet de brancher l'état d'édition et le service de sauvegarde au [mode confirmation du routeur](/docs/routage-confirmation--docs). Le mode confirmation ne sera **activé que si le noeud a été
modifié** (`hasChanged == true`)
