...
De plus, un membre ajouté sera **omis par défaut du résultat de [`toFlatValues`](/docs/modèle-métier-stores-d-entité--docs#toflatvaluesnode-includeaddedfields)**. L'usage principal étant porté par le service de sauvegarde des formulaires, il est plus cohérent d'omettre ces champs qui n'ont par définition pas de persistence dans la modèle. Il est possible cependant de les inclure en renseignant le second paramètre (optionnel) de `toFlatValues` à `true`.
