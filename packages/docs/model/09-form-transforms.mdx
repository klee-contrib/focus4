...
type PatchedUtilisateurEntityType = Patch<
    UtilisateurEntityType,
    {
        ...
        typeUtilisateurCode: FieldEntry<ZodString, TypeUtilisateurCode>; // Champ préexistant modifié.
        liste: ListEntry<
            Patch<
                MySubEntity,
                {
...
- `FieldEntry<ZodString>` => Un champ avec un domaine de schéma `ZodString`.
- `FieldEntry<ZodString, TypeUtilisateurCode>` => Le même champ mais avec un type `string` plus précis.
