...
Comme `edit()`, `required()` dispose également d'une surcharge permettant de passer des propriétés du `StoreNode` en paramètres. Dans ce cas, **la
valeur ou la fonction sera appliquée aux champs/listes/objets demandés au lieu de l'objet en lui-même**. Il est donc parfaitement possible d'utiliser
`required` plusieurs fois, tant que ça s'applique à des propriétés différentes.
...
Pour des sous-noeuds, `required()` appellera le `required()` du builder du sous-noeud, tandis que pour un champ, il posera un
[`metadata({isRequired})`](/docs/modèle-métier-makefield--docs#metadatametadata) ou un [`metadata(() => ({isRequired}))`](/docs/modèle-métier-makefield--docs#metadatametadata), selon le type de `value`.
...
Lorsque vous décrivez un champ (que ça soit dans un `Patch` ou de manière générale), vous avez le choix du niveau de précision dont vous avez besoin.
Idéalement, vous êtes le plus précis possible, mais si vos implémentations n'utilisent pas cette précision vous n'êtes pas obligés d'aller jusqu'au bout.
...
Vous pouvez toujours **baisser la précision du type de champ** au besoin, en particulier dans un `Patch` où vous avez modifié un composant de saisie mais que
vous n'avez pas besoin du détail dans son utilisation.

Dans l'exemple précédent, le type de `typeUtilisateurCode` a été remplacé par `FieldEntry<"string", TypeUtilisateurCode>` dans le `Patch` afin
qu'il reste compatible avec le type modifié dans le `patch` du `useFormNode` par la modification du `SelectComponent`. Attention néanmoins au fait que
**avec cette définition là, le type du `SelectComponent` a été remplacé par `any`**, et que donc le type de `selectProps` ne sera pas vérifié dans l'appel à
`selectFor`. La plupart du temps ce n'est pas très gênant donc ça peut rester un compromis acceptable.

Si vous voulez matérialiser correctement un changement de type de composant sur un champ, les types `PatchInput`, `PatchSelect`, `PatchAutocomplete`,
`PatchDisplay` et `PatchLabel` sont disponibles. Ils s'utilisent par dessus le type du champ avec le type des props du composant en question.

```ts
// Par conséquent, la manière la plus précise de définir le type du champ `typeUtilisateurCode` patché serait ce type-là :
type PatchedTypeUtilisateurCode = PatchSelect<UtilisateurEntityType["typeUtilisateurCode"], SelectRadioProps>;
