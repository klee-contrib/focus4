...
Ce router remplace un [ancien routeur](/docs/autres-modules-focus4∕legacy-routeur--docs), qui utilise des concepts similaires mais ne va pas aussi loin sur la structuration des routes. Les deux routeurs ont vocation a cohabiter puisqu'ils ne sont complètement distincts. Néanmoins, si vous avez le choix, utilisez le plus récent :)
