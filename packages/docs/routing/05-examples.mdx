...
Pour simplifier ce cas très courant, les fonctions `useLoad` et `useFormActions` du module `@focus4/stores` posent une réaction sur les paramètres de leur fonction `load` via le paramètre `params` :
