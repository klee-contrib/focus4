...
Si `coreFetch` reçoit une **réponse avec un code de statut en erreur** (4xx-5xx), et si le serveur renvoie une réponse structurée en JSON, alors elle sera interprétée afin de pouvoir **enregistrer automatiquement des messages d'erreur** (a priori) dans le [`messageStore`](/docs/les-bases-gestion-des-messages--docs).
...
- La méthode `addMessages` du `messageStore` est appelée avec tous les champs non standard présents dans la réponse (`type`, `status`, `title`,
...
Pour rappel, les catégories de messages supportées par le `messageStore` doivent avoir été définies au préalable, si vous voulez gérer autre chose que `error`, `info` ou `warning`. Ce mécanisme permet en revanche de gérer des types de messages personnalisés à partir d'erreurs serveur personnalisées. Cela peut ensuite être utilisé pour différencier la façon dont les erreurs sont restituées à l'utilisateur (au-delà du `MessageCenter` par défaut), par exemple.
...
Par défaut, toute requête faite avec `coreFetch` est automatiquement suivie dans ce store. Les requêtes en cours peuvent être récupérées dans `requestStore.pending`, et la propriété `requestStore.loading` permet de savoir s'il y a au moins une requête encore en cours. Cette propriété peut être utilisée directement pour poser un "spinner" global sur votre application à moindre frais.
...
```ts
const id = useId(); // ou Math.random().toString() si vous n'êtes pas dans un composant
...
**Plusieurs services peuvent être enregistrés sur le même ID de suivi**, ce qui permet de récupérer facilement un état de chargement sur plusieurs services à la fois.
