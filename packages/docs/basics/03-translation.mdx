...
De plus, si vous voulez que la langue choisie dans `i18next` soit automatiquement envoyée au serveur dans toutes les requêtes via le header
`Accept-Language`, vous pouvez renseigner l'option de configuration `coreConfig.useI18nextAcceptHeader` à `true`. Par défaut, ce header est
automatiquement renseigné par le navigateur, et il existe des modules de `i18next` pour détecter automatiquement la langue de l'utilisateur, donc vous
pouvez aussi également gérer le tout de manière automatique 😉.
