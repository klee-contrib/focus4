...
### Listes de références côté client

Si votre liste de référence n'a pas besoin d'être chargée depuis le serveur, vous pouvez aussi la stocker dans un store de référence sous le format :

```ts
const typeDroit = {
    list: [
        {code: "ADMIN", libelle: "Administrateur"},
        {code: "CLASSIC", libelle: "Classique"}
    ], // Il faut remplacer `type` par `list`, avec les valeurs dedans directement.
    valueKey: "code",
    labelKey: "libelle"
};
...
Naturellement, la liste de référence étant "en dur" dans le store, elle est directement disponible et n'a pas besoin d'être chargée...
...
### `makeReferenceList(list, {valueKey, labelKey})` / `makeReferenceList({list, valueKey, labelKey})`
...
Vous pouvez aussi appeler `makeReferenceList` à partir d'une définition de liste de référence, comme celles que vous passez à `makeReferenceStore` (si c'est une définition serveur, avec `type`, la liste sera évidemment vide).
