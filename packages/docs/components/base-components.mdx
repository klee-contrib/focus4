...
La plupart des composants de saisies du module `@focus4/form-toolbox` sont utilisables avec des domaines de plusieurs types de schéma (souvent `string` et `number` par exemple). Lorsque vous les utiliser pour patcher un composant de champ dans un formulaire, il est nécessaire de respécifier ce type en tant que type générique. Dans le cas contraire, vous aurez une erreur pas forcément compréhensible...

Par exemple, pour un `SelectRadio` sur un champ de domaine d'un schéma `string`, il faut écrire :

```tsx
.patch("typeUtilisateurCode", f => f.metadata({SelectComponent: SelectRadio<ZodType<string>>}))
// Vous pouvez aussi utiliser le type Zod précis correspondant au domaine, comme ZodString ou ZodEmail par exemple.
```
