...
_Remarque : Si vous voulez un store local créé dans un composant, vous pouvez utilisez `useLocalCollectionStore` qui l'enregistre dans un `useState` pour vous._
...
_Remarque : Si vous voulez un store local créé dans un composant, vous pouvez utilisez `useServerCollectionStore` qui l'enregistre dans un `useState` pour vous._
...
- `criteria`, un objet de critère personnalisé pour ajouter d'autres champs à utiliser pour la recherche. Cet objet sera stocké sous la forme d'un `FormNode` afin de pouvoir l'utiliser directement avec des champs de formulaire. Il devra donc être défini à partir d'une définition d'entité. Les champs invalides seront retirés de l'appel à la recherche, et la recherche ne sera pas appelée s'il manque un champ obligatoire.
...
```tsx
export const ItemLine = observer(function ItemLine({data, onClick}: {data: Item; onClick?: () => void}) {
