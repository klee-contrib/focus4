...
En mode `local`, il suffit de renseigner la propriété `list` (et `isLoading` si vous voulez suivre son état de chargement dans le store) avec la liste en
question pour le remplir.

_Remarque : En mode `server`, `list` sera renseigné par le serveur et `isLoading` sera géré automatiquement par le store._
...
Le composant [`FacetBox`](/docs/listes-composants-de-recherche-facetbox--docs) permet d'afficher et sélectionner les facettes, tandis que le composant [`Summary`](/docs/listes-composants-de-recherche-summary--docs) affichera la liste des facettes sélectionnées, avec
la possibilité de les retirer. Le composant [`AdvancedSearch`](/docs/listes-composants-de-recherche-advancedsearch--docs) affiche les deux composants.

### Filtrage par critère

Vous pouvez utiliser tous les composants de formulaires avec les champs de critères, afin de pouvoir les renseigner. Vous pouvez les inclure dans une
[`FacetBox`](/docs/listes-composants-de-recherche-facetbox--docs) si vous voulez les ajouter à vos facettes. Le [`Summary`](/docs/listes-composants-de-recherche-summary--docs) affichera la liste des critères non vides, avec la possibilité de les retirer.

### Répartition en groupes

Le composant [`ActionBar`](/docs/listes-composants-de-recherche-actionbar--docs) permet de choisir une facette sur laquelle grouper, et le composant [`Results`](/docs/listes-composants-de-recherche-results--docs) affichera les groupes. Le composant
[`AdvancedSearch`](/docs/listes-composants-de-recherche-advancedsearch--docs) affichera les deux composants.
