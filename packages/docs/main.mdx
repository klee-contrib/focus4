...
Le framework lui-même est aujourd'hui dans sa version majeure **11** (`focus4 v11.x`). Cette documentation n'a vocation qu'à documenter la dernière version
...
fondamentales à signaler (hormis ce qui est documenté dans `@focus4/legacy`). Attendez-vous à avoir des fonctionnalités en moins ou avec des APIs moins
fonctionnelles, tout de même 😁.
