import {Controls, Description, Meta, Title} from "@storybook/addon-docs/blocks";
...
- **`scrollTo(options)`**: même chose que `window.scrollTo`, pour le `<Scrollable>`. Vous pouvez toujours essayer d'utiliser `window.scrollTo` mais ça ne marchera plus, puisque la fenêtre est fixe. C'est a priori la seule raison "standard" (et encore) pour laquelle vous pourriez avoir besoin d'utiliser le contexte.
