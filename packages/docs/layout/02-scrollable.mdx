...
Le `Scrollable` **pose son propre contexte de scroll sur la page**. En particulier, pour celui du [`Layout`](/docs/mise-en-page-layout--docs), il faut bien comprendre que le scroll général de l'application n'est pas celui de le page mais celui du `Scrollable` qui prend 100% de l'espace de l'écran (sauf la partie dédiée au `MainMenu` s'il y'en a un). Il faut bien prendre cela en compte lorsque vous essaierez de personnaliser le CSS général de votre application.
...
-   **`getHeaderStatus()`** : cette méthode permet de connaître si le header est "sticky" et sa hauteur. Cela peut être utilisé pour calculer la position d'éléments sur la page en le prenant en compte (utilisé par le `<ScrollspyContainer>` pour naviguer sur un `<Panel>`).
