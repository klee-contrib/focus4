...
Supposons que l'on ait un composant `panel`, qui a un `content` et un `title`, dont ce dernier peut être positionné en haut (`top`) ou en bas (`bottom`) :
...
_Remarque : malgré l'usage de l'indexeur d'objet, TypeScript valide quand même le nom de la classe._
...
Pour simplifier l'usage des modificateurs et garder le typage fort que les CSS Modules et TypeScript nous apportent, le [générateur de types de Focus](/docs/css-modules-css--docs#typage-des-modules-css) **comprend ce qu'est un modificateur** :
